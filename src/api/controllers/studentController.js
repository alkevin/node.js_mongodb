        ...
        .then(studentToUpdate => {
            if(!studentToUpdate){
                ...
                return res.json({
                    ...
                    studentSent: JSON.parse(req.body)
                });
            }
            Student.updateOne({_id:req.params._id}, {$set: req.body}, (err, result) => {
                if(err){
                    res.status(400);
                    return res.json({
                        status: "400",
                        message: "Could not update student ID: " + req.params.id,
                        studentSent: JSON.parse(req.body),
            ...
            .then(() => {
                res.status(200);
                 return res.json({
                     status: "200",
                     message: "Student with id: " + req.params.id + " updated.",
...
exports.findOneUpdate_student = function(req, res){
    Student.findOneAndUpdate({id:req.params.id},req.body, {new: true, useFindAndModify: false})
    .then(result => {
        if(!result){
            res.status(400);
            res.json({
    ...
    .catch(err => {
        ...
        return res.json({
            status: "500",
            message: "Something wrong updating student with ID: " + req.params.id,
        });
    });
}

exports.delete_student = function(req, res){
    Student.findOne({_id:req.params.id}, {_id: 0})
        .then(student => {
            if(!student){
                res.status(404);
                return res.json({
            ...
            Student.deleteOne({_id:req.params.id}, (err, result) => {
                if(err){
                    res.status(400);
                    return res.json({
                        status: "400",
                        message: "Could not delete student ID: " + req.params.id
                    });
                 }
                 res.status(200);
                 return res.json({
